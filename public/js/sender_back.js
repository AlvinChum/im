...
window.onload = function()
{
	// =====================================================
    function connectweb() {
        ws = new WebSocket("ws://192.168.100.138:3232/");
        ws.onopen = function () {
            // 发送数据
            ws.send(JSON.stringify({"type": "login", "name": "xx", "user_id": user_id}));
        }
    }
    connectweb();
	//当有消息时根据消息类型显示不同信息
	  ...
	  switch(data['type']){
	        ...
	        case 'send':
	        	...
	        	/*if(typeof('show_msg')=="function") {
                    ...
                    alert('找不到方法');
	        		alert('from_client_id:'+data['from_client_id'] + '消息:' +data['content'] + '时间:' + data['time']);
	        	}*/
          case 'ping':
